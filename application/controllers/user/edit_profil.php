...
class edit_profil extends CI_Controller {
	...
	public function update_password($id){
		...
		if($this->form_validation->run()==FALSE){
			...
			$data['password_user'] = $this->model_edit_profil->find($id);
			$this->load->view('user/edit_password_user', $data);
		}else{
			...
			$this->load->model('model_edit_profil');
			$this->model_edit_profil->update_password_m($id, $data_password);
	...
	public function update_profil($id){
		...
		if($this->form_validation->run()==FALSE){
			$this->load->model('model_edit_profil');
			$data['user_profil'] = $this->model_edit_profil->find($id);
			...
			if(!$this->upload->do_upload('foto')){
				...
				$this->load->model('model_edit_profil');
				$this->model_edit_profil->update_profil_m($id, $data_user);
			}else{
